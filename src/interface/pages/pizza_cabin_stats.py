...
def get_data():
    db = client.kafka
    # items = db.all_orders.find({"FACTORY.NAME":"Pizza Cabin"},{"_id":0, "ID":0})
    items = db.all_orders.aggregate(state_totals('Pizza Cabin'))
    items = list(items)
    return items


st.sidebar.markdown("# Pizza Cabin")
st.title("Pizza Cabin")
